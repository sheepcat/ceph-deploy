...
def gatherkeys_with_mon(args, host, dest_dir):
    ...
    #path_keytype_mon = "/Ceph/Data/Mon/mon.%s/keyring" % 
...
def get_admin_keyring(args):
                ...
                if os.path.exists(topath):
                    with open(topath, 'rb') as f:
                        ...
                        if existing != conf_file_contents and not args.overwrite_conf:
                            LOG.error('local keyring file %s exists with different content; use --overwrite-conf to overwrite' % topath)
                            raise
    ...
    raise exc.GenericError('Failed to fetch client.admin.keyring from %d hosts' % errors)
