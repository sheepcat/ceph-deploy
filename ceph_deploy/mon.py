...
def mon_status_check(conn, logger, mon_index, args):
    """
    ...
    """
    asok_path = paths.mon.asok(args.cluster, mon_index)
...
def catch_mon_errors(conn, logger, hostname, mon_index, cfg, args):
    """
    ...
    """
    monmap = mon_status_check(conn, logger, mon_index, args).get('monmap', {})
...
def mon_status(conn, logger, mon_index, args, silent=False):
    """
    ...
    try:
        out = mon_status_check(conn, logger, mon_index, args)
        if not out:
...
def get_mon_no(hostname):
    sections = conf_hd.sections()
    monNo = 'mon.' + hostname
    if monNo not in sections:
        return monNo
    return

def add_mon_conf(servername, publicip):
    mon_no = get_mon_no(servername)

    mon_sec_name = mon_no
    conf_hd.add_section(mon_sec_name)

    conf_hd.set(mon_sec_name, 'host', servername)
    conf_hd.set(mon_sec_name, 'mon addr', publicip + ':6789')
...
def del_conf(conf_name):
    sec_name = conf_name
    conf_hd.remove_section(sec_name)
    conf_write = open(ceph_conf_file, 'w')
    conf_hd.write(conf_write)
    conf_write.close()
    return
...
def mon_add(args):
    ...
    mon_no = get_mon_no(mon_host)
    mon_section = 'mon.%s' % mon_no.split('.')[1]
    ...
    try:
        ...
        args.address = mon_ip
        LOG.debug('Add:add mon to ceph.conf')
        add_mon_conf(mon_host, mon_ip)
        distro.mon.add(distro, args, monitor_keyring, mon_no.split('.')[1])

        # tell me the status of the deployed mon
        time.sleep(2)  # give some room to start
        catch_mon_errors(distro.conn, rlogger, mon_host, mon_no.split('.')[1], cfg, args)
